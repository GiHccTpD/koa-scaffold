...
if (process.env.NODE_ENV === 'development' || process.env.NODE_ENV === 'test') {
    ...
    format = winston.format.combine(
        ...
        winston.format.printf(info => {
            ...
            let result = `${moment().format('YYYY-MM-DD HH:mm:ss.SSS')} [${info.severity.toLowerCase()}]: ${
                info.message
            }`;
...
if (process.env.NODE_ENV === 'production') {
    transports.push(new winston.transports.File({ filename: `log/${moment().format('YYYY-MM-DD')}.log` }));
}
